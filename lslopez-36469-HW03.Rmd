...
```{r}
    ...
    pc1and2 <- -1*eigen_res$vectors
    red_dots  <- cbind(dat %*% (-1*eigen_res$vectors[,1] %*% t(-1*eigen_res$vectors[,1])),
                       rep(1, length(dat[,1])))

    black_dots <- cbind(dat %*% pc1and2, rep(0, length(dat[,1])))
    
    coordinates <- data.frame(rbind(black_dots, red_dots))
    random_projection <- compute_variance(dat, -1*eigen_res$vectors[,1])
    ...
    ggplot(coordinates, aes(color = as.factor(X3))) +
        ...
        labs(x = "Dimension 1",
             y = "Dimension 2",
             color = "Original = 0, Random Projection = 1",
             title = title_pca)
```
